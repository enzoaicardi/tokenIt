...
function addErrors(rule, name){
    listOfRules[name] = rule;
}

function getRule(name){
    return listOfRules[name] || false;
}

function errorIt(type, token, errorTrace, code){
    switch(type){
...
function tokenIt(code, name, tree){
    ...
    var errorFn = rule.errors || errorIt;
    ...
    function explore(object){
        ...
        function errorType(type){
            errorFn(type, token, getTrace(), code);
        }
